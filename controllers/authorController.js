...
exports.author_create_post = function(req, res, next) {
  ...
  // If there are errors:
  if (errors) {
    // Redisplay the form with the entered values and error messages.
    res.render(
      'author_form',
      {title: 'Create Author', author: author, errors: errors}
    );
    return;
  }
  // Otherwise:
  else {
    // The values are valid, so save the author.
    author.save(function (err) {
      // If it could not be saved, report the error.
      if (err) {return next(err);}
      // It was saved, so redirect to the new author’s detail page.
...
exports.author_delete_post = function(req, res, next) {
  ...
  async.parallel({
    ...
    if (err) {return next(err);}
    //Success. If the author is the author of any books:
    ...
    // Otherwise, i.e. if the author is the author of no book:
    else {
      // Delete the author object and redirect to the list of authors.
