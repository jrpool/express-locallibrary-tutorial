...
exports.book_detail = function(req, res, next) {
  ...
  async.parallel({
    ...
    res.render(
      'book_detail',
      {title: 'Title', book: results.book, book_instances: results.book_instance}
    );
  });
...
exports.book_create_post = function(req, res, next) {
  ...
  // If there are errors, and so the book page must be re-rendered:
  if (errors) {
  ...
  // Otherwise, i.e. if there are no errors:
  else {
    // Save the book, without checking whether it is a duplicate.
    book.save(function (err) {
      // If this failed, report the error.
      if (err) {return next(err);}
      // Redirect to the new book record.
...
exports.book_delete_post = function(req, res, next) {
  ...
  async.parallel({
    ...
    books_instances: function(callback) {
      BookInstance.find({'book': req.body.bookid}, 'imprint').exec(callback);
    }
  }, function(err, results) {
    if (err) {return next(err);}
    //Success. If the book is the book of any instance:
    if (results.books_instances > 0) {
    ...
    // Otherwise, i.e. if the book is the book of no instance:
    else {
      // Delete the book object and redirect to the list of books.
